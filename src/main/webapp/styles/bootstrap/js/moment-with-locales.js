...
(function(undefined) {
	...
	globalScope = (typeof global !== 'undefined' && (typeof window === 'undefined' || window === global.window)) ? global
					...
					: m._a[DATE] < 1
							...
							: m._a[HOUR] < 0
									...
									|| (m._a[HOUR] === 24 && (m._a[MINUTE] !== 0
											|| m._a[SECOND] !== 0 || m._a[MILLISECOND] !== 0)) ? HOUR
	...
	function makeAs(input, model) {
		...
		if (model._isUTC) {
			res = model.clone();
			diff = (moment.isMoment(input) || isDate(input) ? +input
					: +moment(input))
					- (+res);
	...
	function getParseRegexForToken(token, config) {
		...
		case 'Do':
			return strict ? config._locale._ordinalParse
	...
	function addTimeToArrayFromToken(token, input, config) {
		...
		case 'Do':
			if (input != null) {
				datePartArray[DATE] = toInt(parseInt(input.match(/\d{1,2}/)[0],
						10));
			}
			break;
		// DAY OF YEAR
	...
	extend(
			moment.fn = Moment.prototype,
			{
				...
				diff : function(input, units, asFloat) {
					var that = makeAs(input, this), zoneDiff = (that
							.utcOffset() - this.utcOffset()) * 6e4, anchor, diff, output, daysAdjust;
				...
				isAfter : function(input, units) {
					...
					if (units === 'millisecond') {
						...
						inputMs = moment.isMoment(input) ? +input
								: +moment(input);
			...
			moment.duration.fn = Duration.prototype,
			{
				...
				as : function(units) {
					...
					if (units === 'month' || units === 'year') {
						...
						days = this._days
								+ Math.round(yearsToDays(this._months / 12));
						switch (units) {
						case 'week':
						...
						case 'second':
							return days * 24 * 60 * 60 + this._milliseconds
									/ 1000;
				...
				return moment
						.defineLocale(
								'af',
								{
									...
									week : {
										dow : 1, // Maandag is die eerste dag
													// van die week.
										doy : 4
									// Die week wat die 4de Januarie bevat is
								...
								'ar-ma',
								{
									...
									week : {
										dow : 6, // Saturday is the first day
													// of the week.
										doy : 12
									// The week that contains Jan 1st is the
						...
						.defineLocale(
								'az',
								{
									...
									weekdays : 'Bazar_Bazar ertəsi_Çərşənbə axşamı_Çərşənbə_Cümə axşamı_Cümə_Şənbə'
											.split('_'),
									weekdaysShort : 'Baz_BzE_ÇAx_Çər_CAx_Cüm_Şən'
											.split('_'),
									weekdaysMin : 'Bz_BE_ÇA_Çə_CA_Cü_Şə'
											.split('_'),
									longDateFormat : {
										LT : 'HH:mm',
										LTS : 'LT:ss',
									...
									ordinal : function(number) {
										if (number === 0) { // special case for
															// zero
											return number + '-ıncı';
										}
										var a = number % 10, b = number % 100
												- a, c = number >= 100 ? 100
												: null;
		...
		factory(moment);
			...
			(function(moment) {
				...
				function monthsCaseReplace(m, format) {
					...
					nounCase = (/D[oD]?(\[[^\[\]]*\]|\s+)+MMMM?/).test(format) ? 'accusative'
							: 'nominative';
						...
						.defineLocale(
								'bn',
								{
									...
									weekdaysShort : 'রবি_সোম_মঙ্গল_বুধ_বৃহস্পত্তি_শুক্রু_শনি'
											.split('_'),
									weekdaysMin : 'রব_সম_মঙ্গ_বু_ব্রিহ_শু_শনি'
											.split('_'),
									longDateFormat : {
										LT : 'A h:mm সময়',
										LTS : 'A h:mm:ss সময়',
									...
									isPM : function(input) {
										return /^(দুপুর|বিকেল|রাত)$/
												.test(input);
									},
									// Bengali is a vast language its spoken
									// in different forms in various parts of
									// the world.
									// I have just generalized with most common
									// one used
									meridiem : function(hour, minute, isLower) {
										if (hour < 4) {
											return 'রাত';
						...
						.defineLocale(
								'br',
								{
									...
									monthsShort : 'Gen_C\'hwe_Meu_Ebr_Mae_Eve_Gou_Eos_Gwe_Her_Du_Ker'
											.split('_'),
									weekdays : 'Sul_Lun_Meurzh_Merc\'her_Yaou_Gwener_Sadorn'
											.split('_'),
			...
			(function(moment) {
				...
				function translate(number, withoutSuffix, key, isFuture) {
					...
					case 'm': // a minute / in a minute / a minute ago
						return withoutSuffix ? 'minuta' : (isFuture ? 'minutu'
								: 'minutou');
					case 'mm': // 9 minutes / in 9 minutes / 9 minutes ago
						if (withoutSuffix || isFuture) {
							return result
									+ (plural(number) ? 'minuty' : 'minut');
						} else {
							return result + 'minutami';
						}
						break;
					case 'h': // an hour / in an hour / an hour ago
						return withoutSuffix ? 'hodina' : (isFuture ? 'hodinu'
								: 'hodinou');
					case 'hh': // 9 hours / in 9 hours / 9 hours ago
						if (withoutSuffix || isFuture) {
							return result
									+ (plural(number) ? 'hodiny' : 'hodin');
						} else {
						...
						.defineLocale(
								'cs',
								{
									...
									monthsParse : (function(months, monthsShort) {
										var i, _monthsParse = [];
										for (i = 0; i < 12; i++) {
											// use custom parser to solve
											// problem with July (červenec)
						...
						.defineLocale(
								'cv',
								{
									...
									weekdays : 'вырсарникун_тунтикун_ытларикун_юнкун_кĕçнерникун_эрнекун_шăматкун'
											.split('_'),
									weekdaysShort : 'выр_тун_ытл_юн_кĕç_эрн_шăм'
											.split('_'),
									weekdaysMin : 'вр_тн_ыт_юн_кç_эр_шм'
											.split('_'),
									longDateFormat : {
										LT : 'HH:mm',
										LTS : 'LT:ss',
				...
				return moment
						.defineLocale(
								'cy',
								{
									...
									// traditional ordinal numbers above 31 are
									// not commonly used in colloquial Welsh
									ordinal : function(number) {
						...
						.defineLocale(
								'el',
								{
									...
									months : function(momentToFormat, format) {
										if (/D/.test(format.substring(0, format
												.indexOf('MMMM')))) { // if
																		// there
																		// is a
																		// day
																		...
																		// before
																		// 'MMMM'
											return this._monthsGenitiveEl[momentToFormat
													.month()];
										} else {
											return this._monthsNominativeEl[momentToFormat
													.month()];
										}
									},
									...
									calendar : function(key, mom) {
										var output = this._calendarEl[key], hours = mom
												&& mom.hours();
									...
									week : {
										dow : 1, // Monday is the first day
													// of the week.
										doy : 4
									// The week that contains Jan 4st is the
									// first week of the year.
									}
								});
			}));
				...
				return moment
						.defineLocale(
								'eo',
								{
									...
									meridiem : function(hours, minutes, isLower) {
										if (hours > 11) {
											return isLower ? 'p.t.m.'
													: 'P.T.M.';
										} else {
											return isLower ? 'a.t.m.'
													: 'A.T.M.';
										}
									},
									calendar : {
		...
		factory(moment);
			...
			(function(moment) {
				var numbersPast = 'nolla yksi kaksi kolme neljä viisi kuusi seitsemän kahdeksan yhdeksän'
						.split(' '), numbersFuture = [ 'nolla', 'yhden',
						'kahden', 'kolmen', 'neljän', 'viiden', 'kuuden',
				...
				function translate(number, withoutSuffix, key, isFuture) {
					...
					switch (key) {
					case 's':
						return isFuture ? 'muutaman sekunnin'
								: 'muutama sekunti';
					case 'm':
				...
				return moment
						.defineLocale(
								'fy',
								{
									...
									monthsShort : function(m, format) {
										if (/-MMM-/.test(format)) {
											return monthsShortWithoutDots[m
													.month()];
										} else {
											return monthsShortWithDots[m
													.month()];
										}
									},
						...
						.defineLocale(
								'gl',
								{
									...
									weekdays : 'Domingo_Luns_Martes_Mércores_Xoves_Venres_Sábado'
											.split('_'),
									weekdaysShort : 'Dom._Lun._Mar._Mér._Xov._Ven._Sáb.'
											.split('_'),
									weekdaysMin : 'Do_Lu_Ma_Mé_Xo_Ve_Sá'
											.split('_'),
									longDateFormat : {
										LT : 'H:mm',
										LTS : 'LT:ss',
				...
				return moment
						.defineLocale(
								'hi',
								{
									...
									// Hindi notation for meridiems are quite
									// fuzzy in practice. While there exists
									// a rigid notion of a 'Pahar' it is not
									// used as rigidly in modern Hindi.
				...
				return moment
						.defineLocale(
								'hu',
								{
									...
									meridiem : function(hours, minutes, isLower) {
										if (hours < 12) {
											return isLower === true ? 'de'
													: 'DE';
										} else {
											return isLower === true ? 'du'
													: 'DU';
										}
									},
									calendar : {
			...
			(function(moment) {
				...
				function translate(number, withoutSuffix, key, isFuture) {
					...
					case 'mm':
						if (plural(number)) {
							return result
									+ (withoutSuffix || isFuture ? 'mínútur'
											: 'mínútum');
					...
					case 'hh':
						if (plural(number)) {
							return result
									+ (withoutSuffix || isFuture ? 'klukkustundir'
											: 'klukkustundum');
						}
					...
					case 'yy':
						if (plural(number)) {
							return result
									+ (withoutSuffix || isFuture ? 'ár'
											: 'árum');
						}
						return result
								+ (withoutSuffix || isFuture ? 'ár' : 'ári');
					}
				}
		...
		factory(moment);
			...
			(function(moment) {
				function monthsCaseReplace(m, format) {
					...
					nounCase = (/D[oD] *MMMM?/).test(format) ? 'accusative'
							: 'nominative';
				...
				function weekdaysCaseReplace(m, format) {
					...
					nounCase = (/(წინა|შემდეგ)/).test(format) ? 'accusative'
							: 'nominative';
						...
						.defineLocale(
								'ka',
								{
									...
									relativeTime : {
										...
										past : function(s) {
											if ((/(წამი|წუთი|საათი|დღე|თვე)/)
													.test(s)) {
												return s.replace(/(ი|ე)$/,
														'ის წინ');
											}
											if ((/წელი/).test(s)) {
												return s.replace(/წელი$/,
														'წლის წინ');
											}
										},
										s : 'რამდენიმე წამი',
	...
	(function(factory) {
		factory(moment);
			...
			(function(moment) {
				 ...
				 * '-n' ending. e.g. 'an 10 Deeg' but 'a 5 Deeg'
				 * 
				 * @param number
				 *            {integer}
				 * @returns {boolean}
				...
				function eifelerRegelAppliesToNumber(number) {
					...
					if (number < 0) {
						...
						// Anything larger than 4 digits: recursively check
						// first n-3 digits
						number = number / 1000;
						return eifelerRegelAppliesToNumber(number);
					}
						...
						.defineLocale(
								'lb',
								{
									...
									weekdays : 'Sonndeg_Méindeg_Dënschdeg_Mëttwoch_Donneschdeg_Freideg_Samschdeg'
											.split('_'),
									weekdaysShort : 'So._Mé._Dë._Më._Do._Fr._Sa.'
											.split('_'),
									weekdaysMin : 'So_Mé_Dë_Më_Do_Fr_Sa'
											.split('_'),
									longDateFormat : {
										LT : 'H:mm [Auer]',
										LTS : 'H:mm:ss [Auer]',
									...
									calendar : {
										...
										lastWeek : function() {
											// Different date string for
											// 'Dënschdeg' (Tuesday) and
											// 'Donneschdeg' (Thursday) due to
											// phonological rule
											switch (this.day()) {
											case 2:
											case 4:
		...
		factory(moment);
			...
			(function(moment) {
				...
				function translateSingular(number, withoutSuffix, key, isFuture) {
					return withoutSuffix ? forms(key)[0]
				...
				function translate(number, withoutSuffix, key, isFuture) {
					...
					if (number === 1) {
						return result
								+ translateSingular(number, withoutSuffix,
										key[0], isFuture);
					} else if (withoutSuffix) {
						return result
								+ (special(number) ? forms(key)[1]
										: forms(key)[0]);
					} else {
						if (isFuture) {
				...
				function relativeWeekDay(moment, format) {
					...
					return nominative ? weekDay : weekDay.substring(0,
							weekDay.length - 2)
							+ 'į';
				}
		...
		factory(moment);
			...
			(function(moment) {
				...
				function format(word, number, withoutSuffix) {
					var forms = word.split('_');
					if (withoutSuffix) {
						return number % 10 === 1 && number !== 11 ? forms[2]
								: forms[3];
					} else {
						return number % 10 === 1 && number !== 11 ? forms[0]
								: forms[1];
					}
				}
				...
				function relativeTimeWithPlural(number, withoutSuffix, key) {
					return number + ' '
							+ format(units[key], number, withoutSuffix);
				}
				...
				return moment
						.defineLocale(
								'ml',
								{
									...
									isPM : function(input) {
										return /^(ഉച്ച കഴിഞ്ഞ്|വൈകുന്നേരം|രാത്രി)$/
												.test(input);
									},
									meridiem : function(hour, minute, isLower) {
										if (hour < 4) {
						...
						.defineLocale(
								'my',
								{
									...
									weekdays : 'တနင်္ဂနွေ_တနင်္လာ_အင်္ဂါ_ဗုဒ္ဓဟူး_ကြာသပတေး_သောကြာ_စနေ'
											.split('_'),
									weekdaysShort : 'နွေ_လာ_င်္ဂါ_ဟူး_ကြာ_သော_နေ'
											.split('_'),
									weekdaysMin : 'နွေ_လာ_င်္ဂါ_ဟူး_ကြာ_သော_နေ'
											.split('_'),
									longDateFormat : {
										LT : 'HH:mm',
										LTS : 'HH:mm:ss',
						...
						.defineLocale(
								'ne',
								{
									...
									weekdaysShort : 'आइत._सोम._मङ्गल._बुध._बिहि._शुक्र._शनि.'
											.split('_'),
									weekdaysMin : 'आइ._सो._मङ्_बु._बि._शु._श.'
											.split('_'),
									longDateFormat : {
										LT : 'Aको h:mm बजे',
										LTS : 'Aको h:mm:ss बजे',
						...
						.defineLocale(
								'nn',
								{
									...
									weekdays : 'sundag_måndag_tysdag_onsdag_torsdag_fredag_laurdag'
											.split('_'),
									weekdaysShort : 'sun_mån_tys_ons_tor_fre_lau'
											.split('_'),
									weekdaysMin : 'su_må_ty_on_to_fr_lø'
											.split('_'),
									longDateFormat : {
										LT : 'HH:mm',
										LTS : 'LT:ss',
								...
								'pl',
								{
									months : function(momentToFormat, format) {
										if (/D MMMM/.test(format)) {
											return monthsSubjective[momentToFormat
													.month()];
										} else {
											return monthsNominative[momentToFormat
													.month()];
										}
									},
						...
						.defineLocale(
								'pt-br',
								{
									...
									calendar : {
										...
										lastDay : '[Ontem às] LT',
										lastWeek : function() {
											return (this.day() === 0 || this
													.day() === 6) ? '[Último] dddd [às] LT'
													: // Saturday + Sunday
													'[Última] dddd [às] LT'; // Monday
																				// -
																				// Friday
										},
										sameElse : 'L'
			...
			(function(moment) {
				...
				return moment.defineLocale('ru', {
					...
					calendar : {
						...
						lastDay : '[Вчера в] LT',
						nextWeek : function() {
							return this.day() === 2 ? '[Во] dddd [в] LT'
									: '[В] dddd [в] LT';
						},
				...
				function translate(number, withoutSuffix, key, isFuture) {
					...
					case 's': // a few seconds / in a few seconds / a few
						...
						return (withoutSuffix || isFuture) ? 'pár sekúnd'
								: 'pár sekundami';
					case 'm': // a minute / in a minute / a minute ago
						return withoutSuffix ? 'minúta' : (isFuture ? 'minútu'
								: 'minútou');
					case 'mm': // 9 minutes / in 9 minutes / 9 minutes ago
						if (withoutSuffix || isFuture) {
							return result
						...
						.defineLocale(
								'sq',
								{
									...
									weekdays : 'E Diel_E Hënë_E Martë_E Mërkurë_E Enjte_E Premte_E Shtunë'
											.split('_'),
									weekdaysShort : 'Die_Hën_Mar_Mër_Enj_Pre_Sht'
											.split('_'),
			...
			(function(moment) {
				...
				return moment.defineLocale('sr-cyrl', {
					...
					calendar : {
						...
						lastDay : '[јуче у] LT',
						lastWeek : function() {
							var lastWeekDays = [ '[прошле] [недеље] [у] LT',
		...
		factory(moment);
			...
			(function(moment) {
				...
				return moment.defineLocale('sr', {
					months : [ 'januar', 'februar', 'mart', 'april', 'maj',
							'jun', 'jul', 'avgust', 'septembar', 'oktobar',
							'novembar', 'decembar' ],
					monthsShort : [ 'jan.', 'feb.', 'mar.', 'apr.', 'maj',
							'jun', 'jul', 'avg.', 'sep.', 'okt.', 'nov.',
							'dec.' ],
					...
					calendar : {
						...
						lastDay : '[juče u] LT',
						lastWeek : function() {
							var lastWeekDays = [ '[prošle] [nedelje] [u] LT',
	...
	(function(factory) {
		factory(moment);
			...
			(function(moment) {
				/*
				 * var symbolMap = { '1': '௧', '2': '௨', '3': '௩', '4': '௪',
				 * '5': '௫', '6': '௬', '7': '௭', '8': '௮', '9': '௯', '0': '௦' },
				 * numberMap = { '௧': '1', '௨': '2', '௩': '3', '௪': '4', '௫':
				 * '5', '௬': '6', '௭': '7', '௮': '8', '௯': '9', '௦': '0' };
				 */
				...
				return moment
						.defineLocale(
								'ta',
								{
									 ...
									 * string.replace(/[௧௨௩௪௫௬௭௮௯௦]/g, function
									 * (match) { return numberMap[match]; }); },
									 * postformat: function (string) { return
				...
				function weekdaysCaseReplace(m, format) {
					...
					nounCase = (/(\[[ВвУу]\]) ?dddd/).test(format) ? 'accusative'
							: ((/\[?(?:минулої|наступної)? ?\] ?dddd/)
									.test(format) ? 'genitive' : 'nominative');
				...
				return moment
						.defineLocale(
								'uk',
								{
									...
									// M. E.: those two are virtually unused but
									// a user might want to implement them for
									// his/her website for some reason
